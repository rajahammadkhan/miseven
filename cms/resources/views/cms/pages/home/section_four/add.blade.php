        ...
        <div class="layout-px-spacing">
            <div class="row layout-top-spacing" id="cancel-row">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('other_pages.home')}}">Section Four</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{$isEdit ? 'Edit' : 'Add'}}</li>
                </ol>
                
                        
                <form action="{{ $isEdit ? route('home_section_four.update', [$home_section_four->id]) : route('home_section_four.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @if ($isEdit)
                    ...
                    <div class="widget-content widget-content-area">
                        <div class="form-row mb-4">
                            <div class="col-md-12">
                                <label>Image <small>(Dimension : 1078 x 660) </small> <span class="text-danger"> *</span> </label>
                                <input {{!$isEdit ? 'required' : null}} type="file" name="image"  class="file" accept="image/*" >
                                <div class="images">
                                    <div class="photo">
                                        <img src="{{url('')}}/uploads/{{$home_section_four->image_one ?? 'placeholder-small.jpg'}}" id="preview" class="img-thumbnail browse">
                                    </div>
                                    @if ($errors->has('icon'))
